...
static int es2ts_initRTPHandler(char *ipaddress, int port, int w, int h, int fps)
{
	char filename[64];
...
int initESHandler(char *ipaddress, int port, int w, int h, int fps)
{
	int ret;

	ret = es2ts_initRTPHandler(ipaddress, port, w, h, fps);
	if (ret < 0)
		return -1;
